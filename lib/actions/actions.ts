...
export const getDogs = async () => {
  const dogs = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/dogs`)
  return await dogs.json()
}

export const getDogDetails = async (dogId: string) => {
  const dog = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/dogs/${dogId}`)
...
export const getOtherDogs = async (dogId: string) => {
  const otherDogs = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/dogs/${dogId}/related`)
  return await otherDogs.json()
}